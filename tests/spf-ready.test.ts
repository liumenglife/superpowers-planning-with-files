...
describe('Command: /spf-ready', () => {
  ...
  it('should deny ready if anchors are missing and notify main agent', () => {
    ...
    expect(result.suggestion).toContain('请主Agent（Main Agent）立即停止其他任务，优先补齐上述缺失');
  });

  it('should allow ready and give normal suggestion if anchors complete and context low', () => {
  ...
  it('should output hard warning if anchors complete and context high', () => {
    fs.mkdirSync(path.join(TEST_DIR, 'docs', 'planning'), { recursive: true });
    fs.writeFileSync(path.join(TEST_DIR, 'docs', 'planning', 'current.md'), '# Current');
    fs.writeFileSync(path.join(TEST_DIR, 'docs', 'planning', 'decisions.md'), '# Decisions');
    fs.mkdirSync(path.join(TEST_DIR, 'docs', 'superpowers', 'specs'), { recursive: true });
