 ...
 */
export function checkRecoveryAnchors(cwd: string): { isComplete: boolean; missing: string[] } {
  ...
  if (fs.existsSync(plansDir) && fs.readdirSync(plansDir).filter(f => f.endsWith('.md')).length > 0) {
    hasSpecOrPlan = true;
  }
