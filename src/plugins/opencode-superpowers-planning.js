...
async function runSpfHook($, directory, percentage) {
  try {
    const result = await $`spf hook ${percentage} opencode`.cwd(directory).quiet().nothrow();
...
export const SuperpowersPlanningPlugin = async ({ directory, $, client }) => {
  return {
    event: async ({ event, session }) => {
      ...
      if (event.type === "session.created" || event.type === "message.created") {
        ...
        if (hookMessage) {
          if (hookMessage.includes("严禁切 Session")) {
            await notify($, "Planning 锚点缺失", "恢复锚点不齐全，大模型正在后台静默为您创建规划文件！");
          } else if (hookMessage.includes("系统状态通知")) {
            await notify($, "Context 警告", `上下文已达 ${percentage}%，建议考虑执行 /new 切换 Session！`);
          }
        }
      }
    },
    ...
    "experimental.chat.system.transform": async (input, output) => {
      ...
      const percentage = await getContextPercentage(client, input.sessionID, knownLimit);
      ...
      if (hookMessage) {
        ...
        if (hookMessage.includes("严禁切 Session")) {
          aiVisualInstruction = `Project anchors are missing! Whether this is a brand new project or an existing one, you MUST silently create the required planning files (docs/planning/current.md, decisions.md, and a spec) right now. If it is an existing project, briefly explore the codebase to infer the current goals and state, and populate the planning files with real content. DO NOT output a red warning to the user or ask them to do it. Just use your file-writing tools to initialize and update these files silently in the background, then proceed with the user's request.`;
