...
switch (command) {
  case "start": {
    const clientType = (args[0] === "opencode" ? "opencode" : "claude-code") as
      | "claude-code"
      | "opencode";
    const output = runSpfStart(cwd, clientType);
    console.log(output);
    break;
  }
  case "ready": {
    const percentage = args[0] ? parseInt(args[0], 10) : 0;
    const clientType = (args[1] === "opencode" ? "opencode" : "claude-code") as
      | "claude-code"
      | "opencode";

    if (isNaN(percentage)) {
      console.error(
        "错误: 请提供一个有效的上下文使用百分比 (0-100)，如: spf ready 45",
      );
      process.exit(1);
    }
  ...
  case "hook": {
    ...
    if (isNaN(percentage)) {
      console.error(
        "错误: 请提供一个有效的上下文使用百分比 (0-100)，如: spf hook 45",
      );
      process.exit(1);
    }
...
用法:
   spf start [client]               初始化 planning 工作流所需的文件和目录
